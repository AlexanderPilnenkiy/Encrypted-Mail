...
namespace MailClient
{
    class Crypto
    {
        ...
        public string Decrypt(string encryptedText, int ID, string user)
        {
            string prKey = File.ReadAllText("C:/KursachMailClient/" + user + "/Private.txt");
        ...
        public string ReturnStringWithTowChipers(string text, int ID, string user)
        {
            string[] temp = ReturnEncryptRijndaelString(text).Split(new string[] { "^&*" }, StringSplitOptions.None);
            /*временный массив для формирования зашифрованного сообщения согласно заданной последовательности*/
            //WorkWithDatabase workWithDatabase = new WorkWithDatabase();
            //string pbKey = workWithDatabase.GetPublicKeyForUser(ID);  /*"Берём public ключ из базы"*/
            string pbKey = File.ReadAllText("C:/KursachMailClient/" + user + "/Public.txt");
